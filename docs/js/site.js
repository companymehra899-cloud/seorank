(function () {
  ...
  function apiPost(endpoint, payload) {
    return fetch(endpoint, {
      ...
      .then(function (response) {
        ...
        if (type.indexOf("application/json") === -1) throw new Error("api-unavailable");
      ...
      .catch(function (error) {
        ...
        if (window.OptirankDemo) return window.OptirankDemo.handle(endpoint, payload);
        throw error;
      });
  }
  ...
  function initAuditForm() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      apiPost("/api/audit", { url: input.value.trim() })
        .then(function (data) {
  ...
  function initNewsletter() {
    document.querySelectorAll("form[data-newsletter]").forEach(function (form) {
      form.addEventListener("submit", function (event) {
        ...
        apiPost("/api/subscribe", { email: email.value.trim() })
          .then(function (data) {
  ...
  function initCookieBanner() {
    ...
    banner.innerHTML = 'We use cookies to analyze traffic and improve your experience. Read our <a href="' + BASE_PATH + '/cookies.html">Cookie Policy</a>. <button class="btn btn-teal" style="height:32px;margin-left:10px" id="cookieOk">Got it</button>';
